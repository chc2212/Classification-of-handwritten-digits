...
if size(samples,2)==1 || maxGain < 0.0001 || min(targets) == max(targets)%|| max(samples) == 0  % only label column left, it's a leaf node
    isLeaf = 1;
...
% samples(1,splitFeatureIdx+1)    % split feature index doesn't include the label column, so here need +1

if splitFeatureIdx<0
    splitFeatureIdx
end

sampleOfSF = samples(:,splitFeatureIdx+1);   % samples of split feature

chosedFeature = splitFeatureIdx;
