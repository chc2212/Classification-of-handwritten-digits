...
elseif strcmp(command,'preprocessing') == 1
    processedTrainDataset = Preprocessing(trainingDataset);
    varargout{1} = processedTrainDataset;
...
elseif strcmp(command,'validation') == 1
    ...
    nVarargs = length(varargin);
    if nVarargs == 2
        Validation(processedTrainDataset, varargin{1}, varargin{2});
    else
        Validation(processedTrainDataset, varargin{1});
    end
